<?php
	include 'header.php';
?>

<!-- box-201-->
<div class="">
	<a class="box-button btn-lg btn-d btn-rd  static-top-button-lft   " style="" href="/guliveriokeliones.php">
		<span class="fa fa-long-arrow-left "> </span>
	</a>
</div>
<div class="">
	<a class="box-button btn-lg btn-d btn-rd    static-top-button-rgt   " style="" href="/izraeliokosmetika.php">
		<span class="fa fa-long-arrow-right "> </span>
	</a>
</div>
<!-- end box-201-->

<!-- box-9 -->
<div class="box bgc-moonstone-blue d-box" id="box-9">
	<div class="container box-sm">
		<div class="row">
<!--			<div class="col-sm-5 col-sm-4 col-sm-53 col-sm-5452 col-sm-54535451 col-sm-5453545254535450"> -->
			<div class="col-sm-3  hidden-md hidden-sm hidden-xs push-top-percents-8 ">
				<img src="img/kym-iphone_home_contacts-3-flat.png"   class="floating-top-down balloon-action pull-right iphone-kymsurgery" style="z-index: 99999999!important; position:absolute;">
<!--				class="pull-right fadeInLeft animDelay04 animated" />-->
			</div>
			<div class="col-sm-12 col-md-12 col-lg-6 push-bottom-percents-3 push-top-percents-3">
				<h1 class="text-center  mg-lg sm-shadow">
					Chirurgijos Klinika
				</h1>
				<img src="img/kym_macbook-falt+stomath2.jpg" class="img-responsive center-block" style="z-index: 1;" />
			</div>
			<div class="col-sm-3 push-top-percents-8  hidden-md hidden-sm hidden-xs">
				<img src="img/kym_iphone-home-1-flat.png"  class="floating-top-down balloon-action iphone-kymsurgery">
<!--					 class="fadeInRight animDelay1 animated" />-->
			</div>
		</div>
	</div>
</div>
<!-- box-9 END -->

<?php
	include('row-atlikome-kymsurgery.html');
?>

<!-- box-11 -->
<div class="box l-box bgc-white push-top-percents-10"  id="box-11">
	<div class="container box-lg">
		<div class="row">
			<div class="col-sm-12">
				<h2 class="text-center  mg-lg ">
					Sukurta Interaktyvi Vartotojo Sąsaja
				</h2>
				<h3 class="text-center  mg-lg  ">
					Užvedus pelę ant organų, kuriems paslaugos teikia įmonė - organai užsidega
				</h3>
				<?php
				include 'button-contact.html';
				?>
			</div>
		</div>
	</div>
</div>

<!-- box-11 -->
<div class="box l-box bgc-white push-bottom-percents-10  hidden-xs hidden-sm " id="box-11">
<!--	<div class="container box-lg">-->
<!--		<div class="row">-->
<!--			<div class="col-sm-12">-->
<!--				<img src="img/kymsurgery_all_body.jpg" class="img-responsive center-block" />-->
<!--			</div>-->
<!--		</div>-->
<!--	</div>-->

	<div class="container-kym box-lg">

		<div class="row">
			<div class="col-sm-12">

				<span id="header_shadow"></span>
				<span id="header_shadow"></span>
				<span id="header_shadow"></span>

				<div class="home-body-image container  test10home">
				</div>

				<div class="interactive-content">
					<div class="header-canvas">
						<div id="header-svg-canvas">
							<object type="image/svg+xml"  data="/active-icons/home-organs-animations.svg">Your browser does not support SVG</object>
							<!--				<img src="/wp-content/themes/enigma-premium/images/slide/home-organs-animations.svg">-->
						</div>
					</div>
				</div>

				<span id="header_shadow_top" class=" rotate-180"></span>
				<span id="header_shadow_top" class=" rotate-180"></span>
				<span id="header_shadow_top" class=" rotate-180"></span>

			</div>
		</div>
	</div>
</div>
<!-- box-11 END -->

<!-- box-12 -->
<div class="box bgc-anti-flash-white l-box  " id="box-12">
	<div class="container box-xxl200">
		<div class="row">
			<div class="col-sm-12">
				<h2 class=" text-center sm-shadow mg-lg">
					Unikalūs Paveikslėliai
				</h2>
				<h3 class="text-center  mg-lg push-bottom-percents-5">
					Projekte panaudoti išskirtinai jam sukurti paveikslėliai
				</h3>
				<img src="img/kym_icons2-grey.png" width="1200" class="img-responsive center-block fadeInUp animDelay08 animated" />
			</div>
		</div>
	</div>
</div>
<!-- box-12 END -->

<!-- box-13 -->
<div class="box bg-dots-bg bg-repeat b-parallax bgc-white-2 l-box hidden-md hidden-sm hidden-xs " id="box-13">
	<div class="container box-xxl300 ">
		<div class="row animDelay02">
			<div class="col-sm-6">
				<img src="img/kym_3-white-2200x1570.jpg" class="center-block img-responsive   fadeInLeft animDelay02 animated" />
			</div>
			<div class="col-sm-6 push-top-percents-8">
				<h2 class=" mg-lg text-center">
					Prisitaikantis Dizainas
				</h2>
				<h3 class=" text-center mg-lg">
					Tinkantis telefonams, plančiatėms,
					<br/> nešiojamiems ir asmeniniams kompiuteriams

				</h3>
				<?php
				include 'button-contact.html';
				?>
			</div>
		</div>
	</div>
</div>
<!-- box-13 END -->
<!-- box-13 -->
<div class="box bg-dots-bg bg-repeat b-parallax bgc-white-2 l-box hidden-lg " id="box-13-mobile">
	<div class="container box-xxl ">
		<div class="row animDelay02">
			<div class=" push-top-percents-8">
				<h2 class=" mg-lg text-center">
					Prisitaikantis Dizainas
				</h2>
				<h3 class=" text-center mg-lg">
					Tinkantis telefonams, plančiatėms,
					<br/> nešiojamiems ir asmeniniams kompiuteriams

				</h3>

				<img src="img/kym_3-white-2200x1570.jpg" class="center-block img-responsive  push-top-percents-10 width-percents-82 fadeInLeft animDelay02 animated" />
			</div>
		</div>
	</div>
</div>
<!-- box-13 END -->

<!-- box-14 -->
<div class="box bgc-white bg--eff-macbook-home-flat d-box b-parallax hidden-sm hidden-xs" id="box-14">
	<div class="container box-xxl300">
		<div class="row">
			<div class="col-sm-12">
				<span class="empty-column"></span>
			</div>
		</div>
	</div>
</div>
<!-- box-14 END -->

<!-- box-15 -->
<div class="box l-box bgc-white" id="box-15">
	<div class="container box-xxl">
		<div class="row">
			<div class="col-sm-12">
				<h2 class=" text-center mg-lg push-bottom-percents-15">
					Tablet Dizainas
				</h2>
				<h3 class=" text-center mg-xl">
				</h3><img src="img/kym_ipad_home_kymsurgery_shadow2_lt.jpg" class="img-responsive center-block mg-sm " />
			</div>
		</div>
	</div>
</div>
<!-- box-15 END -->

<!-- box-16 -->
<div class="box bgc-white l-box  " id="box-16">
	<div class="container box-lg">
		<div class="row">
			<div class="col-sm-6">
				<h2 class="mg-lg  text-center push-top-percents-20">
					Patogūs Kontaktai
				</h2>
				<h3 class=" text-center mg-lg">
					Aiški kontaktų struktūra <br/> ir interaktyvusis Google žemėlapis

				</h3>
				<?php
				include 'button-contact.html';
				?>
			</div>
			<div class="col-sm-6">
				<img src="img/kym_iphone-black-2-contacts-white.jpg" class="img-responsive   floating-top-down" />
			</div>
		</div>
	</div>
</div>
<!-- box-16 END -->

<!-- box-17 -->
<div class="box bgc-white b-parallax" id="box-17">
	<div class="container box-xxl">
		<div class="row">
			<div class="col-sm-12">
				<h2 class="mg-lg text-center  tc-outer-space push-bottom-percents-5">
					Apie Informacija
				</h2><img src="img/kym_eff_2.jpg" class="img-responsive center-block mg-lg" />
			</div>
		</div>
	</div>
</div>
<!-- box-17 END -->

<!-- box-18 -->
<div class="box bgc-white bg-map-overlay d-box" id="box-18">
	<div class="container box-xxl">
		<div class="row">
			<div class="col-sm-12">
				<h2 class=" text-center sm-shadow mg-lg">
					Brangiausiam Turtui – Geriausi Sprendimai
				</h2>
				<?php
				include 'button-contact.html';
				?>
			</div>
		</div>
	</div>
</div>
<!-- box-18 END -->



<?php
include 'footer.html';
?>